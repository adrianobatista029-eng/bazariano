...
export function UserProfileModal({
  ...
  useEffect(() => {
    ...
    Promise.all([
      getSellerPublicStats(supabase, profile.id),
      getBuyerPublicStats(supabase, profile.id),
  ...
  const stats = tab === "seller" ? sellerStats : buyerStats;
  const ordersLabel = tab === "seller" ? "pedidos concluídos" : "compras realizadas";
    ...
    >
      ...
      >
        ...
        <div className="mt-4 flex flex-col items-center gap-3 text-center">
          ...
          <div className="flex w-full rounded-xl bg-secondary/40 p-1">
            <button
              type="button"
              onClick={() => setTab("seller")}
              className={
                tab === "seller"
                  ? "flex-1 rounded-lg bg-brand py-1.5 text-xs font-semibold text-brand-foreground"
                  : "flex-1 rounded-lg py-1.5 text-xs text-muted-foreground"
              }
            >
              Como vendedor
            </button>
          ...
          {stats && stats.top_tags.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1.5">
              {stats.top_tags.map((tag) => (
                <span key={tag} className="rounded-full bg-secondary px-2.5 py-1 text-xs text-foreground">
                  {TAG_LABEL[tag] ?? tag}
                </span>
              ))}
            </div>
