...
export function ModerateButtons({ productId, status }: { productId: string; status: string }) {
  ...
  return (
    <div className="flex gap-2">
      {status !== "active" && (
        <button onClick={() => setStatus("active")} className="text-green-700 underline">
          Ativar
        </button>
      )}
      {status !== "paused" && (
        <button onClick={() => setStatus("paused")} className="text-amber-700 underline">
          Pausar
        </button>
      )}
      {status !== "removed" && (
        <button onClick={() => setStatus("removed")} className="text-red-700 underline">
          Remover
        </button>
      )}
    </div>
